...
import { MessageSender } from '../components/driver/MessageSender'
import { ResetButton } from '../components/driver/ResetButton'
import { RunwayWindView } from '../components/common/RunwayWindView'
...
export function DriverPage() {
  ...
  const { meta, incrementRun, sendTest, resetAll } = useSessionMeta()
  const { logs, getLatestByLocation } = useWindLogs()
  const { params, updateParams } = useTestParams()
  const { history } = useTestHistory()
  const { latestMessage, sendMessage } = useMessage()

  // 現在の走行番号のデータのみ滑走路ビューに表示（番号切替でリセット）
  const latestByLocation = getLatestByLocation(meta.currentRunNumber)
      ...
      <main className="p-4 space-y-4 max-w-xl mx-auto">
        ...
        {tab === 'ダッシュボード' && (
          <>
            ...
            {/* メッセージ送信 */}
            <MessageSender onSend={sendMessage} lastMessage={latestMessage} />

            {/* 滑走路風況ビュー */}
            <RunwayWindView latestByLocation={latestByLocation} />
